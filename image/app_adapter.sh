...
chown -R admin: /home/admin/canal-adapter
host=`hostname -i`
...
function start_adapter() {
    ...
    su admin -c 'cd /home/admin/canal-adapter/bin/ && sh startup.sh 1>>/tmp/start.log 2>&1'
    sleep 5
    #check start
    checkStart "canal" "nc 127.0.0.1 $serverPort -w 1 -zv 2>/tmp/nc.out && cat /tmp/nc.out | grep -c Connected" 30
}

function stop_adapter() {
    echo "stop adapter"
    su admin -c 'cd /home/admin/canal-adapter/bin/ && sh stop.sh 1>>/tmp/start.log 2>&1'
    echo "stop adapter successful ..."
}
